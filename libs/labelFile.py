...
class LabelFile(object):
        ...
        for shape in shapes:
            ...
            if not isRotated:
                ...
                with open(filename[:-4]+'.txt', "a") as f:
                  print("ROBNDBOX: ", robndbox)
                  data = robndbox[0]+','+robndbox[1]+','+robndbox[2]+','+robndbox[3]+','+robndbox[4]+','+robndbox[5]+','+robndbox[6]+','+robndbox[7]+','+str(label)+','+'1'+'\n'
                  f.write(data)
                  '''f.write(data+'\n')
                writer.addRotatedBndBox(robndbox[0],robndbox[1],
                    robndbox[2],robndbox[3],robndbox[4],robndbox[5],robndbox[6],robndbox[7],robndbox[8],label,difficult)
    ...
    def convertPoints2RotatedBndBox(shape):
        ...
        return (str(round(points[0][0])),str(round(points[0][1])),str(round(points[1][0])),str(round(points[1][1])),str(round(points[2][0])),str(round(points[2][1])),str(round(points[3][0])),str(round(points[3][1])))
